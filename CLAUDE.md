...
### 9-12. 세션 시작과 종료 (2026-08-25 확정)

**기준 문서 경로**: `/Users/alex/nirvana_project/hotel roomcheck/PROJECT_STATUS.md`

#### 세션을 시작할 때
1. 위 경로의 `PROJECT_STATUS.md` 를 **먼저 읽는다.** 이 파일이 **최신 상태**다
2. `CLAUDE.md`(이 파일)를 읽어 원칙을 확인한다
3. 문서 내용과 실제 코드가 맞는지 대조한다 — 어긋나면 **코드를 고치지 말고 먼저 보고**한다
4. `PROJECT_STATUS.md` **6절 "다음에 할 일"** 부터 이어서 진행한다
5. 기존 기능은 임의로 지우거나 바꾸지 않는다

#### 세션을 끝낼 때
- 작업이 모두 끝나면 사용자가 `PROJECT_STATUS.md` 를 **직접 업로드한다**
- 따라서 세션 마지막에는 이 문서가 **최신 상태로 갱신돼 있어야 한다**.
  작업을 끝내기 전에 9-11절 규칙대로 갱신하고 커밋한다

#### 업로드본과 디스크본이 다를 때
사용자가 올린 `PROJECT_STATUS.md` 와 폴더 안의 파일이 다르면 **둘 중 하나를 고르지 말고
차이를 먼저 보고한다.** 다른 터미널이나 다른 세션에서 작업이 있었다는 뜻이다.
임의로 덮어쓰면 그쪽 작업이 사라진다.
