...
**최종 갱신**: 2026-08-26 17:10 (이미지 저장 · 공유 링크 · 관리자 목록)

> ⚠️ **`209f089`(관리자 계정 목록)이 push 는 됐으나 배포되지 않았다.**
> Actions 최신 실행이 `3abd23a`(#201)에서 멈춰 있고, 배포본 `admin.html` 에
> 변경이 없다 (`grep -c agency_parent_name` → 0). 3절 참조.
> ⚠️ **「내부 공유용 → 이미지」가 아직 실패한다.** 세 번 고쳤으나 원인 미확정.
> 실패 사유를 화면에 띄우는 버전(`3abd23a`)은 배포돼 있으므로, 사유를 받으면 잡을 수 있다.
...
### 🔴 미해결 ① 「내부 공유용 → 이미지」가 아직 실패한다 (2026-08-26)

세 번 고쳤으나 운영에서 여전히 "이미지를 만들지 못했습니다" 가 난다.
**「견적서만 → 이미지」는 된다.** 둘의 차이는 견적 산출 내역 표와 룸체크 결과 카드다.

지금까지 확인한 것과 고친 것:

| 시도 | 커밋 | 내용 | 결과 |
|---|---|---|---|
| 1차 | `037404e` | 산출 내역 표의 `color-mix` 4곳을 고정색으로 | 여전히 실패 |
| 2차 | `b97aca8` | `flattenColors()` — 뽑기 직전 사본의 색을 전부 `rgb/rgba` 로 | 여전히 실패 |
| 3차 | `3abd23a` | 실패 사유를 화면에 표시 + 캔버스 크기 한계 대응 | **사유 확인 대기** |

원인이었던 것 (확정): 크롬은 `color-mix()` 를 `color(srgb ...)` 로 계산해 내놓고
html2canvas 는 그 표기를 못 읽어 그림 전체를 포기한다. 이 프로젝트에서 네 번 밟았다
(`.qc-dbar` 8/1, `.ntitem` 8/1, `.qbd-*` 8/26, 그리고 전면 평탄화 8/26).
**다만 그것만으로는 해결되지 않았으므로 다른 원인이 더 있다.**

**다음에 할 일: 실패 사유를 받는다.** `3abd23a` 는 배포돼 있으므로
버튼을 누르면 "이미지를 만들지 못했습니다 — <사유>" 가 7초간 뜬다.
개발자도구 콘솔에는 `[이미지] 실패 {폭, 높이, 배율, 오류}` 가 찍힌다.

짚이는 것 (미확인)
- 캔버스 크기 한계 — 아이폰 사파리는 한 변 4096px · 넓이 약 1,670만 px.
  호텔이 여러 개면 세로로 길어져 넘을 수 있다 (`3abd23a` 에서 배율 자동 조정 넣음)
- 추가·추천 호텔이 있는 요청에서만 나는 현상일 가능성 — 테스트 데이터에 없어서 못 걸렀다

### 🔴 미해결 ② `209f089` 이 배포되지 않았다 (2026-08-26)

push 는 정상이다 (`3abd23a..209f089`). 그런데

```
GitHub Actions 최신 실행   #201 = 3abd23a   ← 209f089 실행이 목록에 없다
배포본 admin.html          grep -c agency_parent_name → 0
```

관리자 목록 변경(회사명·합성 이메일)이 운영에 반영되지 않은 상태다.
확인 방법과 수동 배포 명령:
...
⚠️ **모두 컨테이너 검증이다.** 운영(아이폰 사파리·실제 데이터)에서는
「내부 공유용 이미지」가 여전히 실패한다 — 3절 참조. 테스트 요청에 추가·추천 호텔이
없어서 못 걸러낸 전례가 있으므로, 앞으로 이미지 검증은 그 두 가지를 포함해야 한다.
...
0. **🔴 「내부 공유용 → 이미지」 실패 사유를 확인한다** — 3절 미해결 ①.
   `3abd23a` 가 배포돼 있으므로 버튼을 누르면 사유가 화면에 7초간 뜬다.
   그 문구를 가지고 오면 잡을 수 있다. 어떤 요청인지(호텔 개수·추가/추천 호텔 유무),
   PC 인지 아이폰인지도 함께 적는다
0-1. **🔴 `209f089` 배포** — 3절 미해결 ②. Actions 재실행 또는 수동 배포
0-2. **실 부계정으로 로그아웃→재로그인 후 운영 검증** — 우측 상단 소속명 표시,
...
- ~~관리자 목록 회사명·합성 이메일~~ → 코드는 끝났고 **배포만 남았다**
