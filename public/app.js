...
function renderHdrRight(){
  ...
  if(SRV.on){
    items+='<div class="mitem mname">👤 '+esc(SRV.me.name)+'</div>'
      +'<a class="mitem" href="profile.html">'+T('myinfo')+'</a>'
      /* 2026-08-02 [014]: 업무 통계·휴일 근무자를 메뉴로 분리 */
      +((SRV.me.role==='sreq'||SRV.me.role==='schk'||SRV.me.role==='admin')?'<a class="mitem" href="mystats.html">📊 '+T('m_mystats')+'</a>':'')
      +((SRV.me.role==='sreq'||SRV.me.role==='schk'||(SRV.me.role==='admin'&&!SRV.me.super))?'<a class="mitem" href="myoffdays.html">🛌 '+T('m_offday')+'</a>':'')+(SRV.me.role==='admin'?'<a class="mitem" href="admin.html">🏠 관리자</a><a class="mitem" href="request.html">📝 요청자</a><a class="mitem" href="check.html">✓ 확인자</a><a class="mitem" href="agent.html">🤝 에이전트</a>':'')
...
function phoneHTML(req,row){
  ...
  /* 2026-08-02: 이모지는 기기별로 흐리게 보여 흰색 SVG 수화기 아이콘으로 교체 */
  if(sel)h+='<a class="chip on phCall" href="tel:'+sel.replace(/[^+0-9]/g,'')+'" title="'+esc(T('ph_call'))+'" aria-label="'+esc(T('ph_call'))+'">'
    +'<svg viewBox="0 0 24 24" width="16" height="16" fill="#fff" aria-hidden="true"><path d="M6.6 10.8c1.4 2.8 3.8 5.2 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.4.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1C10.6 21 3 13.4 3 4c0-.6.4-1 1-1h3.5c.6 0 1 .4 1 1 0 1.3.2 2.5.6 3.6.1.4 0 .8-.2 1l-2.3 2.2z"/></svg></a>';
...
function renderApp(){
  ...
  /* 2026-08-02 [014]: 휴일 근무자·업무 통계는 메뉴의 별도 페이지로 이동 — 메인은 본인 업무 처리에만 집중 */
...
/* 담당자 후보 (2026-08-02): 해당 에이전트에 '부계정으로 등록된' 담당자만 노출.
   너바나 연락처 담당자·과거 입력 이력은 계정이 없어 알림이 가지 않으므로 목록에서 제외 */
function managerOptions(name){
  const a=AGENTS.find(x=>x.name===name&&x.api&&x.idx);
  const out=[],seen={};
  (AGENT_USERS||[]).forEach(u=>{
    if(!u||!u.name||!u.sub)return;                    // 부계정 계정만
...
/* 담당자 자동 선택 (2026-08-02): 후보는 부계정 목록에서만 나오므로 외부 담당자 조회는 하지 않는다 */
function loadAgencyManagers(name,force){
...
function bindForm(){
  ...
  const am=document.getElementById('agentMgr');
  if(am){am.oninput=e=>{d.agentManager=e.target.value;};
    /* 담당자 드롭다운 (2026-08-01): 화살표/클릭 시 후보 전체 표시, 부계정 연결 여부 함께 표기 */
    attachAC(am,()=>managerOptions(d.agent).map(o=>({label:o.name,s:[o.name]})),
      label=>{d.agentManager=label;am.value=label;});}
